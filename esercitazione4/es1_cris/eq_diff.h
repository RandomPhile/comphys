...
int eu_imp (double h, double *x, double *v) {

   double x_n = *x;
   double v_n = *v;
   
   *x = ( x_n + h*v_n ) / ( 1 + pow(h,2) );
   *v = ( v_n - h*x_n ) / ( 1 + pow(h,2) ); 
...
int trapezi(double h, double *x, double *v) {

   double x_n = *x;
   double v_n = *v;
   
   *x = ( x_n * ( 1 + pow(h,2)/4 ) + h*v_n ) / ( 1 + pow(h,2)/4 );
   *v = ( v_n * ( 1 - pow(h,2)/4 ) - h*x_n ) / ( 1 + pow(h,2)/4 );

   return 0;
}

int eulero_cromer (double h, double *x, double *v ){
