...
int main() {
    ...
    b=0;//questo valore di b non viene usato. era un rimasuglio di una cosa che stavo pensando
    bd=b;
    cd=c;
    
    
    
    for(int i=3; i<10000000;i=i*1.4){//i è un intero e gli assegni il valore di i*1.4. fair enough
        b=i;//valore di un intero assegnato ad un float
        bd=i;//valore di un intero ad un double
        printf("%f\n", b);
        x1=(-b+sqrt(b*b-4*a*c))/(2*a);//mancano parentesi prima di -b grazie
        ...
        x2d=-bd-sqrt(pow(bd,2)-4*ad*cd);//qua immagino fosse x2d
        
        
        err_rel1=fabs((x1d-x1)/x1d)/(2*ad);
        err_rel2=fabs((x1dc-x1c)/x1dc)/(2*ad);
        //manca appunto la soluzione x2... ci interessa quella instabile proprio perche tale
